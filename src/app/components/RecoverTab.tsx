              ...
              <div key={acc.id} className="bg-slate-800/40 border border-slate-700 rounded-2xl overflow-hidden">
                ...
                <div className="p-4">
                  <div className="flex items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400">{acc.contact} · {acc.title} · {acc.industry} · {acc.employees} employees · {acc.country}</p>
                    </div>
                  ...
                  {/* Why now + Open playbook button */}
                  <div className="mt-3 p-3 bg-teal-500/5 border border-teal-500/20 rounded-xl flex items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-teal-400 font-semibold mb-0.5">Why now</p>
                      <p className="text-xs text-slate-300">{acc.why_now}</p>
                    </div>
                    <button
                      onClick={() => setOpenPlaybook(openPlaybook === acc.id ? null : acc.id)}
                    ...
                    >
                      <BookOpen size={12}/>
                      {openPlaybook === acc.id ? 'Close' : 'Open playbook'}
                    </button>
                  </div>
                </div>
                ...
                {openPlaybook === acc.id && (
                  <div className="border-t border-slate-700/50 p-4 space-y-4">
                    {/* Score reasons */}
                    <div>
                      <p className="text-xs text-slate-500 font-semibold mb-2 uppercase tracking-wider">Why this account is scored high</p>
                    ...
                    {/* Email sequence */}
                    <div>
                      <p className="text-xs text-slate-500 font-semibold mb-3 uppercase tracking-wider">3-email reengagement sequence</p>
                      <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                <button
                                  ...
                                  {copied === `${acc.id}-${key}` ? <><CheckCircle size={11}/>Copied</> : <><Copy size={11}/>Copy</>}
                                </button>
                                {i === 0 ? (
                                  <button
                                    onClick={() => handleSend(`${acc.id}-${key}`)}
                                    className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${
                                      sent === `${acc.id}-${key}`
                                        ? 'bg-green-500/20 text-green-400 border border-green-500/30'
                                        : 'bg-indigo-600 hover:bg-indigo-500 text-white'
                                    }`}>
                                    {sent === `${acc.id}-${key}` ? <><CheckCircle size={11}/>Sent ✓</> : <><Send size={11}/>Send</>}
                                  </button>
                                ) : (
                                  <button
                                    onClick={() => handleSend(`${acc.id}-${key}`)}
                                    ...
                                    {sent === `${acc.id}-${key}` ? <><CheckCircle size={11}/>Scheduled ✓</> : <><Send size={11}/>Schedule</>}
                                  </button>
                                )}
                              </div>
                            </div>
