        ...
        <div>
          <div className="grid grid-cols-4 gap-3 mb-5">
            {(['Accelerating','On Track','At Risk','Stalling'] as const).map(s=>{
              ...
              const Icon=st.icon
              return (
                <div key={s} className={`border ${st.border} ${st.bg} rounded-xl p-3`}>
                  <div className="flex items-center gap-2 mb-1"><Icon size={13} className={st.text}/><span className="text-xs text-slate-400">{s}</span></div>
